...
export const Sidebar: React.FC<SidebarProps> = ({ isOpen, setIsOpen, currentView, setCurrentView, currentUser }) => {
        ...
        <div className="p-4 space-y-1">
          ...
          {navItems.map((item) => (
            <button
              ...
              `}
            >
              <item.icon size={20} className={currentView === item.id ? 'text-teal-600 dark:text-teal-400' : 'text-slate-400 dark:text-slate-500'} />
              {item.label}
            </button>
          ))}
        </div>
