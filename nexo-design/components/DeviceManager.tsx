...
import { Plus, Trash2, Wifi, WifiOff, Activity, Laptop, Monitor, Fingerprint, CreditCard } from 'lucide-react';
...
export const DeviceManager: React.FC<DeviceManagerProps> = ({ devices, onAddDevice, onDeleteDevice }) => {
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    if (newDevice.name && newDevice.location) {
      ...
      setNewDevice({ name: '', location: '', type: 'BIOMETRIC', ipAddress: '192.168.1.X', port: '4370' });
    }
  };

  const getDeviceIcon = (type: string, size: number) => {
      switch(type) {
          case 'BIOMETRIC': return <Fingerprint size={size} />;
          case 'RFID': return <CreditCard size={size} />;
          case 'GATE': return <Laptop size={size} />;
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
        {devices.map((device) => (
          <div key={device.id} className="bg-white dark:bg-slate-800 rounded-xl border border-slate-200 dark:border-slate-700 shadow-sm overflow-hidden group">
            <div className="p-5">
              ...
              <p className="text-sm text-slate-500 dark:text-slate-400 mb-4">{device.location}</p>
              ...
              <div className={`flex items-center gap-2 px-3 py-2 rounded-lg text-sm font-medium border ${
                ...
                <span className="flex items-center gap-1.5" title={`Type: ${device.type}`}>
                     ...
                     <span className="text-[10px] uppercase opacity-90 hidden sm:inline-block tracking-wide">{device.type}</span>
                </span>

                <span className="ml-auto text-xs opacity-75 font-normal">Ping: {device.lastPing}</span>
              </div>
            </div>
            <div className="bg-slate-50 dark:bg-slate-700/50 px-5 py-3 border-t border-slate-100 dark:border-slate-700 flex justify-between items-center">
              <span className="text-xs text-slate-500 dark:text-slate-400 font-mono">ID: {device.id.slice(0, 8)}</span>
              <span className="text-xs font-semibold text-slate-600 dark:text-slate-300 font-mono">{device.ipAddress}:{device.port}</span>
            </div>
          </div>
        ))}
      </div>
      ...
      {isModalOpen && (
        <div className="fixed inset-0 bg-black/50 backdrop-blur-sm z-50 flex items-center justify-center p-4">
          <div className="bg-white dark:bg-slate-800 rounded-xl shadow-xl max-w-md w-full p-6 border dark:border-slate-700">
            <h3 className="text-lg font-bold text-slate-800 dark:text-slate-100 mb-4">Add New Device</h3>
