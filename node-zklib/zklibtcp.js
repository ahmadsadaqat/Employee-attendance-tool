...
class ZKLibTCP {
  ...
  executeCmd(command, data) {
    return new Promise(async (resolve, reject) => {
      ...
      if (![COMMANDS.CMD_CONNECT, COMMANDS.CMD_AUTH].includes(command) && !this.is_connect) {
        return reject(new ZKError("instance are not connected"))
      }
  ...
  async getAttendances(callbackInProcess = () => { }) {
    ...
    if (recordData.length % 49 === 0 && recordData.length % 40 !== 0) {
        ...
        let is40Valid = true;
        let is49Valid = true;
        if (recordData.length >= 98) {
            const id40 = recordData.subarray(42, 42 + 9).toString('ascii');
            if (/[^\x20-\x7E\x00]/.test(id40)) is40Valid = false;
            const id49 = recordData.subarray(51, 51 + 9).toString('ascii');
            if (/[^\x20-\x7E\x00]/.test(id49)) is49Valid = false;
            if (is40Valid && !is49Valid) RECORD_PACKET_SIZE = 40;
            else if (is49Valid && !is40Valid) RECORD_PACKET_SIZE = 49;
            else RECORD_PACKET_SIZE = 40;
        } else {
            RECORD_PACKET_SIZE = 40;
        }
    }
    ...
    if (recordData.length % RECORD_PACKET_SIZE !== 0) {
        console.warn(`Warning: Attendance data may be incomplete or corrupt (Length: ${recordData.length}, Size: ${RECORD_PACKET_SIZE})`);
    }
